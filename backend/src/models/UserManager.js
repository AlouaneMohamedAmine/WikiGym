...
class UserManager extends AbstractManager {
  ...
  findAll() {
    return this.connection.query(
      `select id, firstname, lastname, email, is_admin, username from  ${this.table}`
    );
  }

  insert(user) {
    return this.connection.query(
      `insert into ${this.table} (firstname, lastname, email, hashedPassword, is_admin, username ) values (?, ?, ?, ?, ?, ?)`,
      [
        user.firstname,
        user.lastname,
        user.email,
